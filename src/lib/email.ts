...
export async function sendPasswordResetEmail(email: string, token: string): Promise<boolean> {
  ...
  const resend = getResend();
  const subject = 'Reset your TerraceGarden Password';
...
<head>
  <meta charset="utf-8">
  <title>Reset your TerraceGarden Password</title>
  <style>
    body {
      font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Helvetica, Arial, sans-serif;
      background-color: #fcfbf7;
...
<body>
  <div class="container">
    ...
    <div class="text">
      We received a request to reset the password for your TerraceGarden account. To set a new password, click the button below. This link is valid for 1 hour.
    </div>
    <div class="btn-container">
  ...
  try {
    ...
    console.log(`Successfully sent password reset email to ${email}. Message ID: ${data.data?.id}`);
    return true;
  } catch (error) {
    console.error('Resend delivery exception:', error);
    return false;
